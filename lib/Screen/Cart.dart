...
import 'package:sethStore/Helper/Session.dart';
import 'package:sethStore/Helper/SqliteData.dart';

import 'package:sethStore/Provider/CartProvider.dart';
import 'package:sethStore/Provider/SettingProvider.dart';
import 'package:sethStore/Provider/UserProvider.dart';
import 'package:sethStore/Screen/EasypayGateway.dart';
import 'package:sethStore/Screen/PromoCode.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (_isNetworkAvail) {
      ...
      if (payMethod == getTranslated(context, 'COD_LBL')) {
        ...
        final String finalTotal = usedBal > 0
            ? totalPrice.toString()
            : isStorePickUp == "false"
                ? (totalPrice + delCharge).toString()
                : totalPrice.toString();
        ...
        final mobile = await settingsProvider.getPrefrence(MOBILE);
        final email = await settingsProvider.getPrefrence(EMAIL);
        final name = await settingsProvider.getPrefrence(NAME) ?? 'demo';
        final user_id = CUR_USERID;
        final productVariantId = varientId;
        final total = oriPrice.toString();
        final delivery_charge = delCharge.toString();
        final tax_amount = taxPer.toString();
        final tax_percentage = taxPer.toString();
        final final_total = finalTotal;
        final latitude = 40.1451;
        final longitude = -45.4545;
        final promo_code = promocode;
        final address_id = selAddress;
        final deliveryDate = selDate;
        final deliveryTime = selTime;
        final is_wallet_used = isUseWallet! ? "1" : "0";
        final wallet_balance_used = usedBal.toString();
        final active_status = PLACED;
        final order_note = noteC.text;

        final url =
            'https://payment.sethstore.com/easebuzz/app.php?mobile=$mobile&name=$name&email=$email&amount=${totalPrice.toStringAsFixed(0)}&user_id=$user_id&mobile=$mobile&product_variant_id=$productVariantId&quantity=$quantity&final_total=$final_total&promo_code=$promo_code&order_note=$order_note&total=$total&delivery_charge=$delivery_charge&tax_amount=$tax_amount&tax_percentage=$tax_percentage&latitude=$latitude&longitude=$longitude&address_id=$address_id&is_wallet_used=$is_wallet_used&wallet_balance_used=$wallet_balance_used&active_status=$active_status&delivery_date=$deliveryDate&delivery_time=$deliveryTime';
        print(url);
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => EazyPayGateway(
                      myUrl: url,
                    )));
                    ...
                    TextButton(
                        ...
                        onPressed: () async {
                          if (cartList[0].productList![0].productType ==
                              ...
                              if (payMethod ==
                                ...
                                final String finalTotal = usedBal > 0
                                    ...
                                    : isStorePickUp == "false"
                                        ...
                                        : totalPrice.toString();
                                print(finalTotal);
                                print(taxPer.toString());
                                // print(payVia!);
                                print(isUseWallet! ? "1" : "0");
                                print(usedBal.toString());
                                print(noteC.text);
                                print(delCharge.toString());
                                print(selAddress!);
                                print(selDate ?? '');
                                print(emailController.text);
                                print(promocode!);
                                print(promoAmt.toString());
                                print(isStorePickUp == "true" ? "1" : "0");
                                print(PLACED);
                                print(WAITING);
                                SettingProvider settingsProvider =
                                    Provider.of<SettingProvider>(context,
                                        listen: false);
                                final mobile =
                                    await settingsProvider.getPrefrence(MOBILE);
                                final email =
                                    await settingsProvider.getPrefrence(EMAIL);
                                final name =
                                    await settingsProvider.getPrefrence(NAME) ??
                                        'test';
                                final url =
                                    'https://payment.sethstore.com/easebuzz/app.php?mobile=$mobile&name=$name&email=$email&amount=${totalPrice.toStringAsFixed(0)}';
                            ...
                            //   final mobile = await settingsProvider.getPrefrence(MOBILE);
                            //   final email = await settingsProvider.getPrefrence(EMAIL);
                            //   final name = await settingsProvider.getPrefrence(NAME) ??'test';
                            //   final url =
                            //       'https://payment.sethstore.com/easebuzz/app.php?mobile=$mobile&name=$name&email=$email&amount=${totalPrice.toStringAsFixed(0)}&orderid=$Order_id';
                            //
                            //   Navigator.push(
                            //       context,
                            //       MaterialPageRoute(
  ...
  WebViewWidget bankTransfer(String url) {
    controller
